...
// ---- CH4: the lapse and the sweep ------------------------------------------
{
  ...
  const mkFakes = (rows, intent) => {
    ...
    const fakes = {
      ...
      // The third answer (2026-09-09): the real module reads through tryQuery
...
// ---- CH7: the pages ----------------------------------------------------------
{
  ...
  check('CH7 the pages: the client\'s pill reads AWAITING APPROVAL while the hold waits, the card under it says the Worker\'s exact words, a declined case says nothing was charged under the reason, the booking page says the card is held before the button and the return page after, his card carries the tappable amount and posts the decision with it to the route, the shelf tags a case waiting on him, the amount is 44px tall, the seven admin pages load the bumped stylesheet, and what a case paid is what was captured: nothing held, nothing comped, the month on top on the tier, the old receipt on an old case',
    ...
    // stat121 (2026-09-24, v7.8) for ADD/TRIM between PROFIT and LOSS, and to
    // stat122 (2026-09-24, v7.12) for HOLD and SELL on a taken trade, and to
    // stat123 (2026-09-24, v7.13) for the agents' votes, and to
    // stat124 (2026-09-24, v7.14) for the vote folded under a tap, and to
    // stat125 (2026-09-24, v7.15) for the 15-minute chart row on a card.
    ...
    //   FAIL  CH7 the pages: ...
    // NEGATIVE CONTROL (run 2026-09-24, v7.5): admin-desk.html put back to stat118 made this read
    //   FAIL  CH7 the pages: ...
    ...
    //   FAIL  CH7 the pages: ...
    // NEGATIVE CONTROL (run 2026-09-24, v7.12): admin-desk.html put back to stat121 made this read
    //   FAIL  CH7 the pages: ...
    // NEGATIVE CONTROL (run 2026-09-24, v7.13): admin-desk.html put back to stat122 made this read
    //   FAIL  CH7 the pages: ...
    // NEGATIVE CONTROL (run 2026-09-24, v7.14): admin-desk.html put back to stat123 made this read
    //   FAIL  CH7 the pages: ...
    // NEGATIVE CONTROL (run 2026-09-24, v7.15): admin-desk.html put back to stat124 made this read
    //   FAIL  CH7 the pages: ...
    && ['admin-availability', 'admin-calendar', 'admin-case', 'admin-chats', 'admin-dictionary', 'admin', 'admin-desk'].every((p) => /admin\.css\?v=stat125/.test(f(`public/${p}.html`)))
