...
const CONST = {
  ...
  SUB_PRICE_CENTS: num('SUB_PRICE_CENTS'), FULL_PRICE_CENTS: num('FULL_PRICE_CENTS'),
...
async function run(rates) {
  ...
  await fn({});
  return { rates: docs.get('config/rates'), marker: docs.get('migrations/reprice-2026-08-24-tier') };
}

console.log(`constants: full=${CONST.FULL_PRICE_CENTS} case=${CONST.CASE_PRICE_CENTS}\n`);
...
check('R1 a doc nobody has touched gets the full restructure',
  out.rates.caseCents === CONST.CASE_PRICE_CENTS && out.rates.fullCents === CONST.FULL_PRICE_CENTS,
...
check('R5 the RETIRED $3,500 tier price is corrected anyway',
  out.rates.fullCents === CONST.FULL_PRICE_CENTS, `fullCents=${out.rates.fullCents}`);
...
// 3. Idempotent: a finished marker means it never runs twice.
{
  const docs = new Map([
    ...
    ['migrations/reprice-2026-08-24-tier', { finishedAt: new Date(), result: 'done' }],
  ]);
  const deps = {
