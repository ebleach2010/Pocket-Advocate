  ...
  folder = mountFolder({
    ...
    pages: [
      ...
      ] : []),
      {
        ...
        render: (pane) => { paintOverview(pane); paintAuthorityStatus(pane); paintMidwayCard(pane); },
      },
      {
...
// ---- the two-week report card (Eric, 2026-08-30) --------------------------
//
// Generated by itself at day 14 of a Hands-Off month and parked until he
// reads it. His edits in the box ARE the message: Send delivers his text
// word for word, plus the two spreadsheets into the client's Documents.
let midwayMemo = { at: 0, report: null };
let midwayHidden = false;
async function paintMidwayCard(pane) {
  if (!data?.fullAccess || midwayHidden) return;
  if (Date.now() - midwayMemo.at > 60_000) {
    midwayMemo.at = Date.now();
    try {
      const token = await user.getIdToken();
      const res = await fetch(`/api/admin/midway?caseId=${encodeURIComponent(caseId)}`,
        { headers: { authorization: `Bearer ${token}` } });
      midwayMemo.report = res.ok ? (await res.json()).report : null;
    } catch { midwayMemo.report = null; }
  }
  const r = midwayMemo.report;
  if (!r?.generatedAt || r.sentAt) return;
  if (pane.querySelector('[data-midway-root]')) return;
  const box = document.createElement('div');
  box.className = 'panel';
  box.dataset.midwayRoot = '1';
  box.innerHTML = `
    <h3>📬 The two-week report is ready</h3>
    <p class="dim small">Written for ${esc(data.clientName || 'your client')} at the midpoint
      of the month. Read it, make it yours, and Send. It goes out as a chat
      message from you, and two spreadsheets land in their Documents: the
      work log (client lines only) and the whole chat.</p>
    <textarea data-midway-text rows="10" maxlength="2400">${esc(r.draft || '')}</textarea>
    <p class="row" style="gap:.5rem; margin:.5rem 0 0;">
      <button class="btn" data-midway-send>Send it</button>
      <button class="btn ghost" data-midway-later>Not now</button>
    </p>
    <p class="error" data-midway-err hidden></p>`;
  pane.prepend(box);
  box.querySelector('[data-midway-later]').addEventListener('click', () => {
    midwayHidden = true;
    box.remove();
  });
  box.querySelector('[data-midway-send]').addEventListener('click', async (e) => {
    const text = box.querySelector('[data-midway-text]').value.trim();
    const err = box.querySelector('[data-midway-err]');
    if (!text) { err.textContent = 'The message is empty.'; err.hidden = false; return; }
    e.currentTarget.disabled = true;
    err.hidden = true;
    try {
      const token = await user.getIdToken();
      const res = await fetch('/api/admin/midway', {
        method: 'POST',
        headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' },
        body: JSON.stringify({ caseId, action: 'send', text }),
      });
      const out = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(out.error || `Failed (${res.status})`);
      midwayMemo = { at: Date.now(), report: { ...r, sentAt: new Date().toISOString() } };
      box.innerHTML = `<p class="saved-note ok" role="status">Sent. The message is in their
        chat, and both spreadsheets are in their Documents.</p>`;
    } catch (e2) {
