...
export function mountAdvisor({ container, kind, id, user, onSend, draftContainer = null, diffContainer = null, qaContainer = null, goTo = null, self = false, trade = false }) {
  ...
  let lastQa = [];
  // The file a pending question carries, shown by name beside it (2026-09-22).
  let localQFile = null;
  ...
  function renderQa(qa) {
    ...
    const rows = qa.slice(0, 3).slice().reverse().map((q) => `
      <div class="advisor-turn">
        <p class="advisor-q">${esc(q.question)}${q.file ? ` <span class="dim small">📎 ${esc(q.file)}</span>` : ''}</p>
    ...
    if (localQ) rows.push(`
      <div class="advisor-turn">
        <p class="advisor-q">${esc(localQ)}${localQFile ? ` <span class="dim small">📎 ${esc(localQFile)}</span>` : ''}</p>
  ...
  async function submitAsk(question, attachment = null) {
    ...
    localQ = question;
    localQFile = attachment?.name || null;
    renderQa(lastQa);
  ...
  async function carryAct(act) {
    // THE ONE ACT THAT IS NOT A ROUTE. The form sender lives inside
    // admin-case.js and uploads with the client SDK, so there is nothing to
    // POST to and act.path is null. advisor.js cannot import from
    // admin-case.js, so the two speak through a DOM event whose contract both
    // sides wrote down before either could see the other's code.
    //
    // The listener assigns the promise SYNCHRONOUSLY, so a null result after
    // the dispatch means the sender is not on this page at all. That is a
    // different failure from a send that was tried and failed, and it has to
    // say so differently: one is "you are on the wrong screen", the other is
    // "it went wrong".
    if (act.via === 'page') {
      const detail = { kinds: act.kinds || [], result: null };
      document.dispatchEvent(new CustomEvent(act.event, { detail }));
      if (!detail.result)
        throw new Error('The form sender is not on this page. Open the case and try again.');
